...
export default function HairCheckPage() {
  ...
  useEffect(() => {
    const startStream = async () => {
      ...
      try {
        ...
        setErrors((prev) =>
          prev.filter(
            (error) => error.type !== "camera" && error.type !== "microphone",
          ),
        );
      } catch (error) {
        console.error(error);
  ...
  const joinRoom = () => {
    ...
    try {
      ...
      console.error(error);
      setErrors([
        { type: "general", message: "Failed to join room. Please try again." },
      ]);
    }
  };
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            <Alert variant="destructive">
              <AlertDescription>
                {errors.find((error) => error.type === "permission")?.message}
              </AlertDescription>
            </Alert>
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-5xl shadow-none border-0 md:border md:shadow-sm">
        <CardHeader className="space-y-1 pb-4">
          ...
          <CardDescription>
            Make sure everything looks and sounds good before joining
          </CardDescription>
        </CardHeader>
        <CardContent>
