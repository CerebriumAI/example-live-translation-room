...
export default function VideoRoom({
  ...
  const startTranslationService = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        process.env.NEXT_PUBLIC_CEREBRIUM_URL || "",
        {
          method: "POST",
          headers: {
  ...
  React.useEffect(() => {
    ...
    const join = async () => {
      try {
        const result = (await daily.join({
          userName,
          userData: { language: targetLanguage },
  ...
  const getGridLayout = (count: number) => {
    const adjustedCount =
      isTranslationLoading && !translatorFound ? count + 1 : count;
    ...
    <div className="h-screen flex flex-col overflow-hidden relative">
      ...
      <main className="flex-1 min-h-0 p-4">
        <div
          className={cn(
            "grid gap-4 h-full",
            getGridLayout(participantIds.length),
          )}
        >
